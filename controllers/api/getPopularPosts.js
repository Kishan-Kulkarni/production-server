...
const getPopularPosts = async (req, res) => {
  ...
  const rentPosts = await Post.find({ sell: false })
    .sort({ views: -1 })
    .limit(3)
    .exec();
  const rentData = [];
  for (let i = 0; i < rentPosts.length; i++) {
    const currPost = rentPosts[i];
    const currPicture = currPost.pictureURLs[0];
    const result = await getFileStream(currPicture);
    rentData.push([...result.Body]);
  }
  const sellPosts = await Post.find({ sell: true })
    .sort({ views: -1 })
    .limit(3)
    .exec();
  const sellData = [];
  for (let i = 0; i < sellPosts.length; i++) {
    const currPost = sellPosts[i];
    const currPicture = currPost.pictureURLs[0];
    const result = await getFileStream(currPicture);
    sellData.push([...result.Body]);
  }
  return res.json({
    popularPosts: { posts: foundPosts, data: data },
    rentPosts: { posts: rentPosts, data: rentData },
    sellPosts: { posts: sellPosts, data: sellData },
  });
};
